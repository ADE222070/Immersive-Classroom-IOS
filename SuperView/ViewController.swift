    ...
    func loadToolbar() {
        ...
        if let productId = appData?.value(forKey: "RemoveAdsPurchaseId") as? String {
            if productId.isEmpty == true {
                self.removeInAppButton()
            }
        } else {
            self.removeInAppButton()
        }
        
        if Defaults[.adsPurchased] == true  {
            self.removeInAppButton()
        }
    }
    
    func removeInAppButton() {
    ...
    func loadWebSite() {
        ...
        self.wkWebView?.bridge.register({ (parameters, completion) in
            if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
                completion(.success(["version": version]))
            } else {
                completion(.failure(Bridge.JSError.init(code: 503, description: "Can't access version")))
            }
        }, for: "get_app_version")
        ...
        self.wkWebView?.bridge.register({ (parameters, completion) in
            let latitude = LocationService.sharedInstance.lastLocation?.coordinate.latitude ?? 0
            let longitude = LocationService.sharedInstance.lastLocation?.coordinate.longitude ?? 0
            
            completion(.success(["latitude": latitude, "longitude": longitude]))
        }, for: "current_gps_location")
...
//        End of OSC Bridge Test
        ...
        self.wkWebView?.bridge.register({ (parameters, completion) in
            ...
            let url:String! = parameters?["url"] as! String
            
            let activityViewController = UIActivityViewController(activityItems: [title, URL(string: url)!], applicationActivities: nil)
    ...
    func didFinish() {
        ...
        UIView.transition(with: self.view, duration: 0.1, options: .transitionCrossDissolve, animations: {
            ...
            if self.popViewController == nil {
                ...
                if self.bannerView != nil && Defaults[.adsPurchased] == false  {
